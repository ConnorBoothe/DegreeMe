...
const { check, validationResult } = require("express-validator"); //validates user input
//DBs used
          ...
          function (err, match) {
            if (match) {
              ...
              if (user.status === "Active") {
                ...
                listings
                  .getListingsByHandle(req.session.handle)
                  .exec((err, docs) => {
                    if (docs.length > 0) {
                  ...
                  .exec((err, docs) => {
                    for (x in docs) {
                      ...
                      if (new Date(docs[x].date) < new Date()) {
                        req.session.pastConnections.push(
                          new Connection(
                            docs[x]._id,
                            docs[x].tutor,
                            docs[x].img,
                        ...
                        req.session.currentConnections.push(
                          new Connection(
                            docs[x]._id,
                            docs[x].tutor,
                            docs[x].img,
