$(document).ready(function(){
  ...
  $(".textarea").parent().parent().on("focus", function(){
    $(this).css("border", "2px solid #3a3b3c");
  })
  //add new status to the TL
  $(".postStatus").on("click", function(){
    if($(".textarea").text() === "" || $(".textarea").text() === "What's on your mind?"){
      $(".textarea").parent().parent().css("border", "2px solid #dc3545");
    }
    else{
      payload = {
    ...
    $.ajax({
      ...
      headers: {
        ...
        202: function (result) {
        //  $(".helpRequest-body").html('<p class="status-update-success"><svg width="1.5em" height="1.5em" viewBox="0 0 16 16" class="bi bi-hand-thumbs-up" fill="currentColor" xmlns="http://www.w3.org/2000/svg">'+
        ...
        //  '</svg>'+
        //  'Your status update has been published!</p>');
         var htmlToInsert = "<div class='statusUpdateSuccess bg-success'>Status update posted!</div>" + formatStatusUpdate(result.post);
