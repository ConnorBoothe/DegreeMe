            ...
            error:function(err,str){
                ...
                for(var x=res.length-1; x>=0; x--){
                    if(res[x].unreadCount === 0){
                        threads +=  '<a href=../messages'+res[x].threadId+'><li class=" notifications"><div><div class="blue-dot"></div> <img class="notifImg" src="'+res[x].hostImg+'"/><p class="notif">'+res[x].subject+'</p><p class="text-secondary">Created '+displayTimeSince(res[x].timestamp)+'</p></div></a></li>';
                    }
                    else{
                        threads +=  
